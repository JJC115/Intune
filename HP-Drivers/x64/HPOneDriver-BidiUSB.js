...
requestStatus = function (scriptContext, printerStream, printerBidiSchemaResponses) {
    ...
    if ((statusMsg != null) && (statusMsg != "")) {
        if (statusMsg.toUpperCase() == "door-open-error".toUpperCase())
            ...
            printerBidiSchemaResponses.AddString('\\Printer.Status.Summary:StateReason', 'marker-supply-missing-error');
		else if (statusMsg.toUpperCase() == "toner-low-report".toUpperCase())
            printerBidiSchemaResponses.AddString('\\Printer.Status.Summary:StateReason', 'marker-supply-missing-warning');
		else if (statusMsg.toUpperCase() == "toner-low-report".toUpperCase())
            printerBidiSchemaResponses.AddString('\\Printer.Status.Summary:StateReason', 'marker-supply-missing');
		else if (statusMsg.toUpperCase() == "toner-low-report".toUpperCase())
            printerBidiSchemaResponses.AddString('\\Printer.Status.Summary:StateReason', 'marker-supply-failure-error');
		else if (statusMsg.toUpperCase() == "toner-low-report".toUpperCase())
            printerBidiSchemaResponses.AddString('\\Printer.Status.Summary:StateReason', 'marker-supply-failure-warning');
		else if (statusMsg.toUpperCase() == "toner-low-report".toUpperCase())
            printerBidiSchemaResponses.AddString('\\Printer.Status.Summary:StateReason', 'marker-supply-failure');
...
RequestProcessor = function (printerBidiSchemaResponses, pageCache, scriptContext) {
	...
	this.ValidGetFunctions = "GetSimplePrinterAttributes,GetOctetStringValues2,GetOctetStringValues,GetTrayAttributes,GetAttributeString,GetAttributeSubString,GetAttributeValue,MultiStrCompBool,MultiStrCompMultiStr,MultiStrCompString,StrCompBool,GetStatusString,SearchAccessoryBins;GetAttributeFromGroup;GetAttributeFromGroupBool;GetStringFromEnumerations;GetCollectionAttributes";
...
PageCache.prototype.FetchIPPContent = function (targetAttrName) {    
	...
	if (null != targetAttrName) {
        //if (this.VerifyChannel()) // Commenting out to support Aries FW which does not have support to respond with 400 bad request when we send PJL on REST or WinUSB Interface
		{
...
RequestProcessor.prototype.GetAttributeSubString = function (bidiQueryStr, func, queryArray) {
    var attrValue = "";
    if (this.ValidGetFunctions.indexOf(func) != -1)
        attr = this[func](queryArray);
    else
...
RequestProcessor.prototype.MultiStrCompMultiStr = function (bidiQueryStr, func, queryArray) {
	var result = false;
	var attrValue = "";
	var len = 0;
	var count = 0;
	...
	if (attr != null) {
		...
		for (var i in queryArray) {
			if (result == true) {
				attrValue = tempArray[1];
				break;
			}
			var tempArray = queryArray[i].split('=');
			var temp = tempArray[0].split(",");
			for (var innerIndex in attr.Values) {
				attrIndex = attr.Values[innerIndex];
				for (var outerIndex in temp) {
					if (temp[outerIndex] == attrIndex) count++;
				}
			}
